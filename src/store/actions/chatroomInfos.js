...
// 用于demo记录封面
export function initChatroomInfos({
    state,
    commit
}, obj) {
        ...
        chatroom.getChatroom({
                ...
                chatroom.getChatroomMembersInfo({
                    accounts: [creator],
                        ...
                        commit('getChatroomInfo', Object.assign(info, {
                            actor: user.members[0]
                        }))
                    }
                })
            }
...
export function getChatroomMembers({
    state,
    commit,
    dispatch
}) {
    ...
    getChatroomMembersLocal(false, function (obj) {
        commit('updateChatroomMembers', Object.assign(obj, {
            type: 'put'
        }))
        // 再拉成员列表
...
export function clearChatroomMembers({
    state,
    commit
}) {
    commit('updateChatroomMembers', {
        type: 'destroy'
    })
}
